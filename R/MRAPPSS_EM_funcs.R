...
cal_likeli <- function(hat.b, A, Var, S, C1, C2, pi0){
  ...
  Sigma <- mapply(function(M1,M2){M1 +  A %*% M2 %*% t(A) }, M1=S, M2=Var)
  Sigma = lapply(split(Sigma, col(Sigma)), function(x){matrix(x, nrow=2, ncol=2)})

  P1 =  mapply(mvtnorm::dmvnorm, x= hat.b,  sigma=Sigma)
...
MRAPPSS_EM_func <- function(data = NULL,
  ...
  s11 = se.exp^2 * drop(Sigma_err[1,1])
