...
MR-APSS uses a foreground-background model to characterize the estimated effects of SNPs $G_j$ on $X$ and $Y$ ($\hat{\gamma}_j$ and $\hat{\Gamma}_j$)  with standard errors ($\hat s_{X,j}, \hat s_{Y,j}$), where the background model accounts for polygenicity, correlated pleiotropy (\textbf{B}) and sample structure (\textbf{C}), and the foreground model (\textbf{A}) aims to identify informative instruments and account for uncorrelated pleiotropy to perform causal inference. %More details on MR-APSS can be found in the Materials and Methods section.
